  ...
  return (
    <section id="projects" className="py-32 bg-[#fdfafb] dark:bg-[#080508] relative transition-colors duration-500">
      <div className="max-w-6xl mx-auto px-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-20">
          <div className="space-y-4">
            ...
            <p className="text-gray-600 dark:text-white/50 max-w-md font-light">
          ...
          <div className="flex gap-4">
            <button
              onClick={() => scroll('left')}
              className="p-4 rounded-2xl bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white hover:border-purple-500/50 hover:bg-purple-500/5 transition-all duration-300 group"
      ...
      <div className="relative">
        <div
          ref={scrollRef}
          className="flex overflow-x-auto gap-8 pb-12 snap-x snap-mandatory scrollbar-hide scroll-smooth px-[max(1.5rem,calc((100vw-72rem)/2+1.5rem))]"
        >
          {PROJECTS.map((project, i) => (
            <motion.div
              ...
              className="group cursor-pointer relative flex-shrink-0 w-[85vw] md:w-[500px] aspect-[4/3] rounded-[3rem] overflow-hidden bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 shadow-sm hover:shadow-xl transition-all duration-500 snap-center"
            >
              <img
                src={project.image}
                alt={project.title}
