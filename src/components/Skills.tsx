      ...
      <div className="max-w-6xl mx-auto px-6 relative z-10">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-12 mb-24">
          <div className="space-y-6">
            ...
            <p className="text-white/50 max-w-md text-xl font-light leading-relaxed">
          ...
          <div className="flex flex-wrap gap-3">
            {categories.map((cat) => (
              <button
                ...
                className={`px-8 py-3 rounded-2xl text-xs font-mono uppercase tracking-widest transition-all duration-500 border ${
                  activeCategory === cat
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredSkills.map((skill, i) => (
            <motion.div
              ...
              key={skill.name}
              className="group relative p-10 glass-panel rounded-[3rem] hover:border-purple-500/50 transition-all duration-500 overflow-hidden"
            >
              ...
              <div className="relative z-10 space-y-8">
                <div className="flex items-start justify-between">
                  ...
                  <div className="w-12 h-12 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-white/20 group-hover:text-purple-400 group-hover:border-purple-500/30 transition-all duration-500">
                    {skill.category === 'AI / ML' ? <Zap size={20} /> : skill.category === 'Frontend' ? <Code size={20} /> : <Database size={20} />}
                  </div>
                </div>

                <div className="pt-6 flex flex-wrap gap-3">
                  {skill.relatedProjects.map(proj => (
                    <span key={proj} className="text-[10px] font-mono text-purple-400/40 uppercase tracking-widest px-4 py-1.5 rounded-xl bg-purple-500/5 border border-purple-500/10 group-hover:border-purple-500/30 group-hover:text-purple-400 transition-colors">
                      {proj}
                    </span>
                  ))}
                </div>
